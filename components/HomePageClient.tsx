...
export interface HomePageClientProps {
  initialHomepageContent?: Partial<HomepageData>;
  initialCategories?: Category[];
  initialCategoryThumbnails?: Record<string, string>;
  initialServices?: Service[];
  initialTopProducts?: any[];
  initialMostSellingProducts?: any[];
}

export default function HomePageClient({
  initialHomepageContent,
  initialCategories,
  initialCategoryThumbnails,
  initialServices,
  initialTopProducts,
  initialMostSellingProducts
}: HomePageClientProps) {
  const router = useRouter();
  const { settings } = useSiteSettings();

  const [categories, setCategories] = useState<Category[]>(initialCategories || []);
  const [categoryThumbnails, setCategoryThumbnails] = useState<Record<string, string>>(initialCategoryThumbnails || {});
  const [loading, setLoading] = useState(!initialCategories?.length);


  // Homepage content from database
  const [homepageContent, setHomepageContent] = useState<Partial<HomepageData>>(initialHomepageContent || defaultHomepageContent);
  const [imageKey, setImageKey] = useState(0);

  // Services state - fetch from database
  const [services, setServices] = useState<Service[]>(initialServices || []);
  const [servicesLoading, setServicesLoading] = useState(!initialServices?.length);

  // Fetch homepage content from database if not provided
  useEffect(() => {
    if (initialHomepageContent) return; // Skip if provided via SSR
  ...
  useEffect(() => {
    if (initialServices && initialServices.length > 0) return; // Skip if provided via SSR
    ...
    const fetchServices = async () => {
      try {
        ...
        setServices([
          { _id: '1', title: 'Custom Manufacturing', description: 'We create custom steel furniture tailored to your specific requirements and space.', icon: '🏭', isActive: true, order: 1 } as any,
          { _id: '2', title: 'Free Consultation', description: 'Our experts provide free consultation to help you choose the perfect furniture.', icon: '💬', isActive: true, order: 2 } as any,
          { _id: '3', title: 'Installation Service', description: 'Professional installation service included with every purchase.', icon: '🔧', isActive: true, order: 3 } as any,
          { _id: '4', title: 'After-Sales Support', description: '5-year warranty and dedicated after-sales support for all products.', icon: '🛡️', isActive: true, order: 4 } as any,
          { _id: '5', title: 'Bulk Orders', description: 'Special pricing for bulk orders for offices, schools, and institutions.', icon: '📦', isActive: true, order: 5 } as any,
          { _id: '6', title: 'Fast Delivery', description: 'Quick delivery across Biratnagar, Dharan, Itahari and nearby areas.', icon: '🚚', isActive: true, order: 6 } as any
        ]);
      } finally {
  ...
  // Fetch categories and their thumbnails if not provided
  useEffect(() => {
    if (initialCategories && initialCategories.length > 0) return; // Skip if provided via SSR
  ...
  return (
    <div>
      ...
      <CleanTopProductsSection initialProducts={initialTopProducts} />

      {/* Most Selling Products Section */}
      <CleanMostSellingSection initialProducts={initialMostSellingProducts} />

      {/* Google Reviews Section */}
      <section className="py-16 bg-gray-50/50 border-t border-b border-gray-100">
        <div className="content-container text-center max-w-3xl mx-auto px-6">
          <div className="flex justify-center items-center gap-1 mb-3.5">
          ...
          <h2 className="text-xl md:text-2xl lg:text-3xl font-extrabold text-gray-900 mb-3.5">
            हाम्रा ग्राहकहरूको वास्तविक प्रतिक्रिया हेर्नुहोस्
          </h2>
          <p className="text-gray-600 text-sm md:text-base mb-6 leading-relaxed max-w-2xl mx-auto">
            हामी गुणस्तरीय स्टील फर्निचर निर्माण र सेवामा प्रतिबद्ध छौं। हाम्रो गुगल बिजनेस प्रोफाइल (Google Business Profile) मा ग्राहकहरूले दिनुभएको वास्तविक समीक्षाहरू पढ्न वा आफ्नो समीक्षा लेख्न तलको बटनमा क्लिक गर्नुहोस्।
          </p>
          <a
            href="https://www.google.com/maps/place/Shree+Manish+Steel+Furniture+Udhyog+Pvt.+Ltd./@26.49980678332793,87.27763091503517,17z/data=!3m1!4b1!4m6!3m5!1s0x39ef7395d46084a5:0xc709a12df1274cc8!8m2!3d26.49980678332793!4d87.27763091503517!16s%2Fg%2F11y3k8y8y8"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2.5 bg-white hover:bg-gray-50 text-gray-800 font-semibold px-6 py-3 rounded-lg border border-gray-300 shadow-sm transition-all hover:scale-102"
          >
            ...
            गुगल समीक्षाहरू हेर्नुहोस् (Google Reviews)
          </a>
        </div>
      </section>
